...
const DataTable: React.FC = () => {
	const [data, setData] = useState<DataItem[]>([
		{ id: 1, date: new Date(), time: '12:00', Amonia: 10, Temperatura: 20, Umidade: 30 },
		{ id: 2, date: new Date(), time: '15:30', Amonia: 15, Temperatura: 25, Umidade: 35 },
	...
	const columns: Column<DataItem>[] = useMemo(
		() => [
			...
			{
				Header: 'Data e Hora',
				accessor: (row: DataItem) => format(new Date(row.date), 'dd/MM/yyyy HH:mm'),
			},
	...
	const filteredData = useMemo<DataItem[]>(
		() =>
			data.filter(
				(row) =>
					(dateRange.startDate ? new Date(row.date) >= dateRange.startDate : true) &&
					(dateRange.endDate ? new Date(row.date) <= dateRange.endDate : true) &&
